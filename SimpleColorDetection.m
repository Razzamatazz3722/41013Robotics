...
% With new versions of MATLAB, you can go to the Apps tab and use the "Color Thresholder - RGB" app to do the same thing.
...
try
	...
	axis on;
	hp = impixelinfo();
	set(hp,'units', 'Normalized', 'Position',[.15, .95, .2, .020]);
	...
	hp = impixelinfo();
	set(hp,'units', 'Normalized', 'Position',[.2, .35, .2, .020]);
...
function PlaceThresholdBars(plotNumber, lowThresh, highThresh)
	...
	hStemLines = stem([lowThresh highThresh], [maxYValue(2) maxYValue(2)], 'r', 'MarkerSize', 0.001);
